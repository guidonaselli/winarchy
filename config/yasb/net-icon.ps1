# net-icon.ps1 — computa el glifo de red para el widget "wifi" de YASB.
#
# Decide el glifo de forma SINCRONICA (sin el WiFiWorker/winrt de yasb.wifi.WifiWidget,
# que deja el placeholder {wifi_icon} literal cuando no puede leer el adaptador: red abajo
# en el arranque o radio WiFi off). Siempre devuelve un glifo, con fallback a
# "desconectado", asi que la barra nunca muestra la plantilla cruda.
#
# NO usar Get-NetAdapter: es CDXML/CIM y tarda ~1 s por invocacion (medido). System.Net.
# NetworkInformation es .NET puro y resuelve lo mismo en ~30 ms.
#
...
#
# Solo la funcion: la corre en loop scripts\Start-NetIcon.ps1, que la dot-sourcea. YASB
# ya no invoca este archivo directo (ver ese script para el porque).

function Get-WinarchyNetIconGlyph {
